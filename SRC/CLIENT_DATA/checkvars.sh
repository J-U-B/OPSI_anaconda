...
#
# Version: 1.2.0
# Jens Boettge <boettge@mpi-halle.mpg.de>  	2022-10-19 17:40:10 +0200
#=======================================================================
...
VARS_USED=$(cat *.opsi{inc,script}{,.in} 2>/dev/null| grep -v '^\s*;' | grep -i 'set\s*\$' | sed -e 's/^\s*//' | sed -e 's/\t/ /' | sed -re 's/^.*set\s+/set /i' | cut -f 2 -d " " | sort | uniq)
